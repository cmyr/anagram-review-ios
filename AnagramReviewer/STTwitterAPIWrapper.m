...
@property (nonatomic, strong) NSObject <STTwitterOAuthProtocol> *oauth;
@end
...
+ (STTwitterAPIWrapper *)twitterAPIWithOAuthOSX {
    ...
    twitter.oauth = [[STTwitterOAuthOSX alloc] init];
    return twitter;
}
...
+ (STTwitterAPIWrapper *)twitterAPIApplicationOnlyWithConsumerKey:(NSString *)consumerKey
    ...
    STTwitterAppOnly *appOnly = [[STTwitterAppOnly alloc] init];
...
- (void)getTimeline:(NSString *)timeline
	...
	__weak NSMutableArray *statuses = [NSMutableArray new];
...
-(void)getStatusWithID:(NSString *)statusID
    ...
    NSDictionary *d = @{@"id": statusID, @"include_entities": (entities ? @"true" : @"false")};
    [_oauth getResource:@"statuses/show.json" parameters:d successBlock:^(id response) {
...
- (void)postStatusUpdate:(NSString *)status
    ...
    NSMutableDictionary *md = [ @{ @"status":status, @"media[]":data, @"postDataKey":@"media[]" } mutableCopy];
